...
class GraphicsGraph(QGraphicsScene):
    ...
    def __init__(
        ...
        self._scene = scene
        self._uuid = graph.uuid()
        graph.component_registered.subscribe(self.register_component)
        # graph.component_unregistered.subscribe(self.unregister_component)
        # graph.port_registered.subscribe(self.register_port)
        # graph.port_unregistered.subscribe(self.unregister_port)
        # graph.connection_registered.subscribe(self.register_connection)
        # graph.connection_unregistered.subscribe(self.unregister_connection)

        self._graphics_components: List[UUID] = []
        self._graphics_ports: List[UUID] = []
        self._graphics_connections: List[UUID] = []
    ...
    def register_component(self, component: Component) -> None:
        ...
        """
        graphics_component = self._scene.get_graphics_component(component)

        self._graphics_components.append(component.uuid())
        self.addItem(graphics_component)

        logger.debug("Registered graphics component %s", component.path())
